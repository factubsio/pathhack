...
### Warpriest
Divine martial. Blessings, fervor (cooldown-based), sacred weapon. Partial caster (5-level, limited slots at top). Deity choice at level 1 determines favored weapon and blessing options.

bit of archery-ness if favoured weapon is bow like thing

### Oracle
Divine full caster. Mystery + curse at level 1. Spells half-choice, half-random (mystery forces some on you). Curse is drawback that grows with power. (as does msytery)
...
have to do some fun bloodlines to make this good
...
how do we do reactions?

ammo will be joyful to iplement - I was thinking we let lainchers can always launch ammo at like +0 (maybe even -N) but quivered is gooder? need to noodle

### Monk
Martial. Ki pool, stances. Focus at level 1:
...
don't nerf the otehr options on pickup, druid we will probably penalise the non-order paths (a bit?) but monk should just be more tools for the given pick. don't think should be damage based stuff, but utility (basically are you more flexible in punch or bow, and sensei is many more "support" stuff)
...
maybe decrease faster the higher it is
...
- Assassin/Rogue (sneak attack, poison - open to any class)
- Ranger (favored enemy/terrain)
- Hunter (teamwork, animal focus)
...
- Full (9-level equivalent): Oracle, Sorcerer, Druid
- Partial (6-level equivalent): Magus, Alchemist, Warpriest
- None: Swashbuckler, Monk, Kineticist (own systems), bit conceredn swashhy will be left behind

non-pf people will be tres confused that monk is the archer but iiwii
