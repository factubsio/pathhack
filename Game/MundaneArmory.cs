namespace Pathhack.Game;

[GenerateAll("AllWeapons", typeof(WeaponDef))]
[GenerateAll("AllArmors", typeof(ArmorDef))]
public static partial class MundaneArmory
{
    public static readonly WeaponDef Dagger = new()
    {
        id = "dagger",
        Name = "dagger",
        BaseDamage = d(4),
        Style = WeaponStyle.Simple, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Dagger,
        DamageType = DamageTypes.Piercing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 10,
        Price = 2,
    };

    public static readonly WeaponDef Shortsword = new()
    {
        id = "shortsword",
        Name = "shortsword",
        BaseDamage = d(6),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Shortsword,
        DamageType = DamageTypes.Slashing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 30,
        Price = 9,
    };

    public static readonly WeaponDef Longsword = new()
    {
        id = "longsword",
        Name = "longsword",
        BaseDamage = d(8),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Longsword,
        DamageType = DamageTypes.Slashing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 40,
        Price = 10,
    };

    public static readonly WeaponDef Scimitar = new()
    {
        id = "scimitar",
        Name = "scimitar",
        BaseDamage = d(6),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Scimitar,
        DamageType = DamageTypes.Slashing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 40,
        Price = 10,
    };

    public static readonly WeaponDef Rapier = new()
    {
        id = "rapier",
        Name = "rapier",
        BaseDamage = d(6),
        Style = WeaponStyle.Skewer, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Rapier,
        DamageType = DamageTypes.Piercing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 30,
        Price = 20,
    };

    public static readonly WeaponDef Falchion = new()
    {
        id = "falchion",
        Name = "falchion",
        BaseDamage = d(2, 4),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Falchion,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 80,
        Price = 30,
    };

    public static readonly WeaponDef Club = new()
    {
        id = "club",
        Name = "club",
        BaseDamage = d(6),
        Style = WeaponStyle.Simple, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Club,
        DamageType = DamageTypes.Blunt,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 30,
        Price = 1,
    };

    public static readonly WeaponDef SpikedClub = new()
    {
        id = "spiked_club",
        Name = "spiked club",
        BaseDamage = d(8),
        Style = WeaponStyle.Simple, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Club,
        DamageType = DamageTypes.Blunt,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 40,
        Price = 2,
    };

    public static readonly WeaponDef Greatclub = new()
    {
        id = "greatclub",
        Name = "greatclub",
        BaseDamage = d(10),
        Style = WeaponStyle.Impact, Grip = WeaponGrip.Great,
        WeaponType = WeaponTypes.Greatclub,
        DamageType = DamageTypes.Blunt,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 120,
        Price = 10,
    };

    public static readonly WeaponDef Mace = new()
    {
        id = "mace",
        Name = "mace",
        BaseDamage = d(6),
        Style = WeaponStyle.Impact, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Mace,
        DamageType = DamageTypes.Blunt,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 40,
        Price = 10,
    };

    public static readonly WeaponDef Flail = new()
    {
        id = "flail",
        Name = "flail",
        BaseDamage = d(6),
        Style = WeaponStyle.Flail, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Flail,
        DamageType = DamageTypes.Blunt,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 50,
        Price = 8,
    };

    public static readonly WeaponDef Quarterstaff = new()
    {
        id = "quarterstaff",
        Name = "quarterstaff",
        BaseDamage = d(6),
        Style = WeaponStyle.Staff, Grip = WeaponGrip.Great,
        WeaponType = WeaponTypes.Quarterstaff,
        DamageType = DamageTypes.Blunt,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 40,
        Price = 1,
    };

    public static readonly WeaponDef BoStaff = new()
    {
        id = "bo_staff",
        Name = "bo staff",
        BaseDamage = d(8),
        Style = WeaponStyle.Staff, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.BoStaff,
        DamageType = DamageTypes.Blunt,
        Hands = 2,
        Reach = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 50,
        Price = 2,
    };

    public static readonly WeaponDef Spear = new()
    {
        id = "spear",
        Name = "spear",
        BaseDamage = d(6),
        Style = WeaponStyle.Simple, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.Spear,
        DamageType = DamageTypes.Piercing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Launcher = "hand",
        MeleeVerb = "thrust",
        Weight = 30,
        Price = 1,
    };

    public static readonly WeaponDef Scythe = new()
    {
        id = "scythe",
        Name = "scythe",
        BaseDamage = d(10),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.Scythe,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 100,
        Price = 20,
    };

    public static readonly WeaponDef Halberd = new()
    {
        id = "halberd",
        Name = "halberd",
        BaseDamage = d(10),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.Halberd,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Reach = 2,
        MeleeVerb = "chops at",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 120,
        Price = 25,
    };

    public static readonly WeaponDef Glaive = new()
    {
        id = "glaive",
        Name = "glaive",
        BaseDamage = d(8),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.Glaive,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Reach = 2,
        MeleeVerb = "slashes",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 100,
        Price = 15,
    };

    public static readonly WeaponDef LucerneHammer = new()
    {
        id = "lucerne_hammer",
        Name = "lucerne hammer",
        BaseDamage = d(10),
        Style = WeaponStyle.Impact, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.LucerneHammer,
        DamageType = DamageTypes.Blunt,
        Hands = 2,
        Reach = 2,
        MeleeVerb = "smashes",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 120,
        Price = 20,
    };

    public static readonly WeaponDef Ranseur = new()
    {
        id = "ranseur",
        Name = "ranseur",
        BaseDamage = d(8),
        Style = WeaponStyle.Skewer, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.Ranseur,
        DamageType = DamageTypes.Piercing,
        Hands = 2,
        Reach = 2,
        MeleeVerb = "thrusts",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 100,
        Price = 15,
    };

    public static readonly WeaponDef Bardiche = new()
    {
        id = "bardiche",
        Name = "bardiche",
        BaseDamage = d(12),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Polearm,
        WeaponType = WeaponTypes.Bardiche,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Reach = 2,
        MeleeVerb = "cleaves",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 140,
        Price = 30,
    };

    public static readonly WeaponDef Greatsword = new()
    {
        id = "greatsword",
        Name = "greatsword",
        BaseDamage = d(2, 6),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Great,
        WeaponType = WeaponTypes.Greatsword,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        MeleeVerb = "slashes",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 80,
        Price = 50,
    };

    public static readonly WeaponDef EarthBreaker = new()
    {
        id = "earth_breaker",
        Name = "earth breaker",
        BaseDamage = d(2, 6),
        Style = WeaponStyle.Impact, Grip = WeaponGrip.Great,
        WeaponType = WeaponTypes.EarthBreaker,
        DamageType = DamageTypes.Blunt,
        Hands = 2,
        MeleeVerb = "smashes",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 140,
        Price = 40,
    };

    public static readonly WeaponDef Whip = new()
    {
        id = "whip",
        Name = "whip",
        BaseDamage = d(4),
        Style = WeaponStyle.Exotic, Grip = WeaponGrip.Exotic,
        WeaponType = WeaponTypes.Whip,
        DamageType = DamageTypes.Slashing,
        Reach = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 20,
        Price = 1,
    };

    public static readonly WeaponDef SpikedChain = new()
    {
        id = "spiked_chain",
        Name = "spiked chain",
        BaseDamage = d(8),
        Style = WeaponStyle.Flail, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.SpikedChain,
        DamageType = DamageTypes.Piercing,
        Reach = 2,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkGray),
        Weight = 100,
        Price = 30,
    };

    public static readonly WeaponDef Longbow = new()
    {
        id = "longbow",
        Name = "longbow",
        BaseDamage = d(8),
        Style = WeaponStyle.Exotic, Grip = WeaponGrip.Exotic,
        WeaponType = WeaponTypes.Longbow,
        DamageType = DamageTypes.Blunt, //for whacking!
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 30,
        Price = 60,
        NotForWhacking = true,
    };

    public static readonly WeaponDef Blowgun = new()
    {
        id = "blowgun",
        Name = "blowgun",
        BaseDamage = d(2),
        Style = WeaponStyle.Exotic, Grip = WeaponGrip.Exotic,
        WeaponType = WeaponTypes.Blowgun,
        DamageType = DamageTypes.Blunt,
        Hands = 1,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkGreen),
        Weight = 5,
        Price = 10,
        NotForWhacking = true,
    };

    public static readonly WeaponDef Dart = new()
    {
        id = "dart",
        Name = "dart",
        BaseDamage = d(4),
        Style = WeaponStyle.Thrown, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Dart,
        DamageType = DamageTypes.Piercing,
        Stackable = true,
        Launcher = "hand",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 2,
        Price = 1,
        NotForWhacking = true,
    };

    // Is this for whacking?
    public static readonly WeaponDef Bola = new()
    {
        id = "bola",
        Name = "bola",
        BaseDamage = d(6),
        Style = WeaponStyle.Thrown, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Bola,
        DamageType = DamageTypes.Blunt,
        Launcher = "hand",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 20,
        Price = 5,
    };

    public static readonly WeaponDef Hatchet = new()
    {
        id = "hatchet",
        Name = "hatchet",
        BaseDamage = d(6),
        Style = WeaponStyle.Simple, Grip = WeaponGrip.Light,
        WeaponType = WeaponTypes.Hatchet,
        DamageType = DamageTypes.Slashing,
        Launcher = "hand",
        Stackable = true,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 20,
        Price = 4,
    };

    public static readonly WeaponDef Battleaxe = new()
    {
        id = "battleaxe",
        Name = "battleaxe",
        BaseDamage = d(8),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Axe,
        DamageType = DamageTypes.Slashing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 60,
        Price = 10,
    };

    public static readonly WeaponDef DwarvenWaraxe = new()
    {
        id = "dwarven_waraxe",
        Name = "dwarven waraxe",
        BaseDamage = d(10),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Axe,
        DamageType = DamageTypes.Slashing,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.White),
        Weight = 80,
        Price = 30,
    };

    public static readonly WeaponDef Greataxe = new()
    {
        id = "greataxe",
        Name = "greataxe",
        BaseDamage = d(12),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Great,
        WeaponType = WeaponTypes.Greataxe,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 120,
        Price = 20,
    };

    [NotRandomlyGenerated]
    public static readonly WeaponDef Pickaxe = new()
    {
        id = "pickaxe",
        Name = "pickaxe",
        BaseDamage = d(6),
        Style = WeaponStyle.Skewer, Grip = WeaponGrip.Heavy,
        WeaponType = WeaponTypes.Pick,
        DamageType = DamageTypes.Piercing,
        MeleeVerb = "strike",
        Glyph = new(ItemClasses.Weapon, ConsoleColor.Gray),
        Weight = 60,
        Price = 14,
        Components = [DiggerIdentity.Instance, DiggerVerb.Instance],
    };

    public static readonly WeaponDef Gandasa = new()
    {
        id = "gandasa",
        Name = "gandasa",
        BaseDamage = d(2, 4),
        Style = WeaponStyle.Carve, Grip = WeaponGrip.Great,
        WeaponType = WeaponTypes.Greataxe,
        DamageType = DamageTypes.Slashing,
        Hands = 2,
        Glyph = new(ItemClasses.Weapon, ConsoleColor.DarkYellow),
        Weight = 80,
        Price = 15,
    };

    public static readonly ArmorDef LeatherArmor = new()
    {
        id = "leather_armor",
        Name = "leather armor",
        Proficiency = Proficiencies.LightArmor,
        ACBonus = 1,
        DexCap = 4,
        Glyph = new(ItemClasses.Armor, ConsoleColor.DarkYellow),
        Components = [new ArmorBrick(1, 4)],
        Weight = 25,
        Price = 20,
    };

    public static readonly ArmorDef ChainShirt = new()
    {
        id = "chain_shirt",
        Name = "chain shirt",
        Proficiency = Proficiencies.LightArmor,
        ACBonus = 2,
        DexCap = 3,
        CheckPenalty = -1,
        Glyph = new(ItemClasses.Armor, ConsoleColor.Gray),
        Components = [new ArmorBrick(2, 3)],
        Weight = 50,
        Price = 50,
    };

    public static readonly ArmorDef HideArmor = new()
    {
        id = "hide_armor",
        Name = "hide armor",
        Proficiency = Proficiencies.MediumArmor,
        ACBonus = 3,
        DexCap = 2,
        CheckPenalty = -2,
        Glyph = new(ItemClasses.Armor, ConsoleColor.DarkYellow),
        Components = [new ArmorBrick(3, 2)],
        Weight = 50,
        Price = 20,
    };

    public static readonly ArmorDef Breastplate = new()
    {
        id = "breastplate",
        Name = "breastplate",
        Proficiency = Proficiencies.MediumArmor,
        ACBonus = 4,
        DexCap = 1,
        CheckPenalty = -2,
        Glyph = new(ItemClasses.Armor, ConsoleColor.Gray),
        Components = [new ArmorBrick(4, 1)],
        Weight = 100,
        Price = 80,
    };

    public static readonly ArmorDef SplintMail = new()
    {
        id = "splint_mail",
        Name = "splint mail",
        Proficiency = Proficiencies.HeavyArmor,
        ACBonus = 4,
        DexCap = 1,
        CheckPenalty = -3,
        Glyph = new(ItemClasses.Armor, ConsoleColor.DarkGray),
        Components = [new ArmorBrick(4, 1)],
        Weight = 150,
        Price = 130,
    };

    public static readonly ArmorDef FullPlate = new()
    {
        id = "full_plate",
        Name = "full plate",
        Proficiency = Proficiencies.HeavyArmor,
        ACBonus = 6,
        DexCap = 0,
        CheckPenalty = -3,
        Glyph = new(ItemClasses.Armor, ConsoleColor.White),
        Components = [new ArmorBrick(6, 0)],
        Weight = 250,
        Price = 300,
    };

    public static readonly ItemDef RingOfKnives = new()
    {
        id = "ring_of_knives",
        Name = "ring of knives",
        Glyph = new(ItemClasses.Ring, ConsoleColor.Cyan),
        DefaultEquipSlot = ItemSlots.Ring,
        Components = [new GrantProficiencyWhenEquipped(Proficiencies.LightBlade, ProficiencyLevel.Trained)],
        Weight = 3,
        Price = 400,
    };
}
